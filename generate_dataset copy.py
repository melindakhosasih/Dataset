    ...
    with habitat.Env(config=config, dataset=dataset) as env:
        for epi in (tqdm(range(env.number_of_episodes))):
            ...
            for step, path in enumerate(current_episode.shortest_paths[0]):
                ...
                if path.position != env.sim.get_agent_state().position.tolist() or path.rotation != quat_to_coeffs(env.sim.get_agent_state().rotation).tolist():
                    obs = env.sim.get_observations_at(path.position, path.rotation, True)
